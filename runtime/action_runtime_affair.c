...
/* note_add / affair_close / defer mutate affair state through ordinary
 * runtime actions. Each checks its own preconditions (for example, an
 * active affair_review) and reports failures through the action boundary. */
...
int fc_note_add(RtRun *r, const char *rid, const RtActionDef *def,
  ...
  char etext[RT_LARGE * 2], eaff[RT_MED];
...
int fc_counter_bump(RtRun *r, const char *rid, const RtActionDef *def,
  ...
  char eaff[RT_MED], ename[32 * 6 + 1], payload[RT_LARGE];
  long long value = 0;
  ...
  if (rt_append_event_format(
          ...
          eaff, ename, has_delta ? "delta" : "set", value) != 0) {
    snprintf(error, error_len,
...
int fc_affair_close(RtRun *r, const char *rid, const RtActionDef *def,
                           const char *args, char *result, size_t result_len,
                           char *error, size_t error_len) {
  char eaff[RT_MED], ectx[RT_MED];
  char payload[RT_LARGE];
  (void)rid; (void)def; (void)args;
...
int fc_affair_open(RtRun *r, const char *rid, const RtActionDef *def,
  ...
  if (rt_append_event_format(
          ...
          "\"manage\":\"%s\",\"status\":\"active\",\"next_review_at_ms\":%lld}",
          eaff, ectx, emanage, next_review_at_ms) != 0) {
    snprintf(error, error_len,
             "{\"message\":\"failed to append affair_patch event\"}");
    snprintf(result, result_len, "{}");
    return -1;
